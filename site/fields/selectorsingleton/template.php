...
<?php if ($field->files() && $field->files()->count() > 0): ?>
    <div class="input input-with-items">
        ...
        <?php foreach ($field->files() as $file): ?>
        	...
        	<?php
	        	...
	        	if(preg_match("=_shared/(.*)/=", $file->url(), $temp)){ $filename = "teaserpool/". $filename; }
	        ?>
            <div id="<?= $field->itemId($file) ?>" class="item item-with-image [ selector-item js-selector-item ]" data-file="<?=$filename ?>" <?= r($field->isInValue($filename), 'data-checked="true"') ?> >
